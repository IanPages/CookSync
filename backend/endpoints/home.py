...
from schemas.home_schemas import HomeCreateRequest, HomeJoinRequest, HomeUpdateRequest, HomeKickUserRequest, HomeLeaveRequest
...
async def create_home(payload:HomeCreateRequest,current_user:User = Depends(get_current_user),db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        print("new home after refresh:", new_home)
...
async def leave_home(payload: HomeLeaveRequest,current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    try:
        ...
        if existing_member.role == "owner":
            stmt= select(HomeMember).where(HomeMember.home_id == payload.home_id)
            result= await db.execute(stmt)
...
@router.put("/kick", status_code=status.HTTP_200_OK)
...
@router.get("/{home_id}/members", status_code=status.HTTP_200_OK)
async def get_home_members(home_id: UUID, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    try:
        ...
        stmt = select(User, HomeMember.role, HomeMember.joined_at).join(HomeMember, User.id == HomeMember.user_id).where(HomeMember.home_id == home_id)
        result = await db.execute(stmt)
        members_data = []
        for row in result.all():
            user_obj, role, joined_at = row
            members_data.append({
                "id": str(user_obj.id),
        ...
        logger.error(f"Error fetching home members: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="An error occurred while fetching home members."
        )
